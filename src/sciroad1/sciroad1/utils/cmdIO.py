...
class StepConfig(BaseConfig):
    def __init__(self):
        ...
        #是否采样
        self.parser.add_argument('--sampling_flag', type=bool, default=True)
...
class TrackingConfig(BaseConfig):
    def __init__(self):
        ...
        # 减速加速度，米/秒^2
        self.parser.add_argument('--dec', type=float, default=8)

        # 最大速度阈值，米/秒，超过15会丢步
        self.parser.add_argument('--v', type=float, default=12)
...
def io_set_rx(args):
    ...
    qq = input("请手动设置发射机参数。Config文件中配置当前频率为%sGHz，当前功率为%sdBm, 倍频为%s，若正确请敲回车，若不正确请输n：" % (
        args.freq, args.power, args.multi))
    if qq == 'n' or qq == 'N':
        freq, power, multi = input("请手动设置发射机参数。设置当前的频率、功率和倍频，只写数字，默认单位为GHz和dBm，空格隔开：").split()
    else:
        freq = args.freq
        power = args.power
        multi = args.multi
